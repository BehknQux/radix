...
<div class="container">
    ...
    <div class="d-flex">
        <button v-for="button in roadmaps" :key="button.label" @click="active = button.label" :class="['btn', 'text-light', 'border-0', 'px-4', 'py-2', 'rounded-5', 'mx-1', { 'btn-dark': active === button.label }]">
            {{ button.label }}
        </button>
    </div>
    <div class="my-5 mx-4 position-relative">
        <div class="line position-absolute" ref="line">
            <div class="progress-bar bg-white" :style="{ height: getPercentOfProgress() + '%' }"></div>
        </div>
        <div class="row roadmap-item" v-for="(rm, index) in activeRoadmapData" :key="index" :class="{
            ...
            <div class="col-1 d-flex flex-column align-items-center">
                <div class="bg-black dot-container">
                    <p></p>
                    <div class="border border-2 rounded-circle dot d-flex align-items-center justify-content-center">
...
export default {
        ...
        return {
            ...
            roadmaps: [{
                    ...
                    data: [{
                            ...
                            bottom_text: 'DOCK tokens will be supported in this self-custodial iOS/Android application.',
                            checked: false,
                        },
                        {
                            title: 'DOCK Token Integration on the Nova Wallet',
    ...
    computed: {
        activeRoadmapData() {
            const activeRoadmap = this.roadmaps.find((rm) => rm.label === this.active);
            return activeRoadmap ? activeRoadmap.data : [];
        },
    },
    methods: {
        getElementOffsetTop() {
            this.offsetTop = this.$refs.line.getBoundingClientRect().top + window.scrollY;
        },
        getElementOffsetBottom() {
            this.offsetBottom = this.$refs.line.getBoundingClientRect().bottom + window.scrollY;
        },
        getCenterPoint() {
        ...
        getPercentOfProgress() {
            let percent = ((this.centerPoint - this.offsetTop) * 100) / (this.offsetBottom - this.offsetTop);
            if (percent < 100) {
                return percent;
            } else {
                return 100;
